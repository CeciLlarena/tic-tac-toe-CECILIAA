...
def decide_move(board: list, player_id: str) -> list:
    ...
    if board [1][1] == "-":
        return [1,1]
    ###Si ya esta tomada, que elija la posicion de arriba a la izquierda
    if board [1][1] == enemigo and board [0][0] == "-":
        return [0,0]

    
### Si me toco de primero, significa que esatoy en la posicion del medio.
### Calcular mi siguiente movimiento siemore yendo a izquiera arriba
    if board [1][1] == player_id and board [0][0] == '-':
        return [0,0]
    
    


###Si soy 0, en el segundo turno defender, DEFENDER en el segundo turno. Bloquear cuando estan dos juntos, y cuando hay un espacio en medio
    if board [0][0] == enemigo and board [0][1] == enemigo and board [0][2] == '-':
        return [0,2]
    if board [0][2] == enemigo and board [0][1] == enemigo and board [0][0] == '-':
        return [0,0]
    ...
    if board [2][1] == enemigo and board [1][1] == enemigo and board [0][1] == '-':
        return [0,1]
    if board [0][1] == enemigo and board [2][1] == enemigo and board [1][1] == '-':
        return [1,1]
...
###para esquinas
    ...
    if board [2][2] == enemigo and board [1][1] == enemigo and board [0][0] == '-':
        return [0,0]
    if board [2][2] == enemigo and board [0][0] == enemigo and board [1][1] == '-':
        return [1,1]
    if board [0][2] == enemigo and board [1][1] == enemigo and board [2][0] == '-':
        return [2,0]
    if board [2][0] == enemigo and board [1][1] == enemigo and board [0][2] == '-':
        return [0,2]
    if board [0][2] == enemigo and board [2][0] == enemigo and board [1][1] == '-':
        return [1,1]
...
###Si empiezo en las esquinas y tengo un hoyo en medio
